...
def thesis_page(cards: dict[str, Card], statement: str, question: str) -> str:
    roles = [
        ...
        ("Objet et réification", "idea_0165", "Ce qui devient intéressant peut devenir un objet pour le sujet, puis être réifié si cette naissance est oubliée."),
...
<section class="page-hero">
  <div class="shell page-hero__inner">
    ...
    <p class="page-hero__lead">L'intéressant comme pouvoir relationnel de déclencher chez un sujet un processus de construction.</p>
  </div>
</section>
<section class="section">
  <div class="shell thesis-page-grid">
    ...
    <article class="prose prose--large">
      <section id="objet"><p class="eyebrow">Objet</p><h2>Constituer l'intéressant comme objet philosophique.</h2><p>Le projet ne traite pas l'intéressant comme un synonyme vague de préférence, de nouveauté ou de beauté. Il cherche à décrire un type de relation entre une forme et un sujet doté d'une mémoire, d'attentes et de capacités acquises.</p><div class="question-callout"><span>Question directrice</span><strong>{html.escape(question)}</strong></div><div class="button-row"><a class="button button--primary" href="../downloads/projet-these-court-fr.pdf">Télécharger la version courte (PDF)</a></div></section>
      <section id="hypothese"><p class="eyebrow">Hypothèse centrale actuelle</p><blockquote>{html.escape(statement)}</blockquote><p>Le terme décisif est <em>construction</em> : modifier sa manière de percevoir, de questionner, de comprendre ou d'agir en distinguant, reliant, modélisant, essayant ou produisant autrement. La définition porte sur le déclenchement ; elle ne suppose pas que le processus réussisse.</p><p><em>L'intérescence</em> est le nom proposé pour la dynamique ainsi ouverte : rupture de l'indifférence, construction, transformations, puis épuisement, reprise ou transmission. Le néologisme reste un outil à éprouver.</p><p>Une hypothèse nouvelle porte sur la naissance de l'objet : ce qui devient intéressant peut se détacher du fond et devenir un objet pour le sujet. La réification commence lorsque l'on oublie que cet objet est né d'une relation. Cette proposition vise d'abord la réification vécue, car des objets administratifs ou automatiques peuvent s'imposer sans intérêt préalable.</p><p>Cette hypothèse reste un candidat. Sa force dépendra de sa capacité à distinguer une construction effective d'une fascination sans capacité nouvelle, puis à résister aux cas limites.</p></section>
      <section id="plan"><p class="eyebrow">Plan de travail</p><h2>Trois parties provisoires.</h2><p>À la suite d'une discussion avec Olivia Chevallier, co-directrice de la thèse, le travail est organisé provisoirement en trois parties.</p><div class="method-list">{plan_html}</div><p>L'étude de Garve comprend un projet de traduction française intégrale de son essai, mené avec Christian Berner pour les éditions Vrin. Les textes philosophiques aideront à préciser le concept, à formuler des objections et à montrer ce qu'il apporte.</p></section>
      ...
      <section id="methode"><p class="eyebrow">Mise à l'épreuve</p><h2>Préciser le concept par des cas.</h2><p>Chaque hypothèse sera comparée à des explications rivales et confrontée à des objections. La musique et l'intelligence artificielle/création fourniront les deux principaux terrains d'étude. Un résultat scientifique éclairera la réflexion philosophique sans suffire, à lui seul, à la prouver.</p><div class="method-list"><div><strong>Musique</strong><span>Attente, mémoire et micro-transformations de l'attention.</span></div><div><strong>IA et création</strong><span>Construction d'artefacts, génération/jugement et substitution de cible.</span></div><div><strong>Analyse intérescentielle</strong><span>Proust pour la trajectoire longue, <em>Bartleby</em> pour la raréfaction et Job pour la motivation sans intérescence interne identifiable.</span></div></div><p>Le cas de Job impose une distinction importante : une motivation morale et affective très forte ne prouve pas qu'un objet devienne intéressant pour le personnage. La souffrance, la demande de justice et l'orientation vers Dieu peuvent mouvoir Job sans ouvrir pour lui de nouvelles questions ; l'énigme devient alors intéressante pour le lecteur. Un vrai contre-exemple religieux devrait montrer un objet non prescrit qui ouvre pour un personnage des questions successives et le transforme.</p></section>
    </article>
  </div>
</section>
"""
...
def build(output: Path) -> None:
    ...
    short_thesis_pdf = ROOT / "docs" / "projet-these-court-fr.pdf"
    if short_thesis_pdf.is_file():
        destination = output / "downloads" / short_thesis_pdf.name
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(short_thesis_pdf, destination)
