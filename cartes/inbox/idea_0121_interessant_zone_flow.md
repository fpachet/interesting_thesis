...
- L'augmentation des compétences devrait déplacer vers des tâches plus difficiles la zone jugée intéressante.
- L'ennui et l'anxiété devraient pouvoir être réduits en modifiant soit la tâche, soit les compétences disponibles.
- Une mesure de l'intérêt devrait estimer un rapport sujet-tâche plutôt qu'une propriété isolée de l'objet.
...
- Généralise la géométrie de l'ennui décrite dans `idea_0004`.
- Isolé de `idea_0050` la proposition sur le rapport compétences-difficulté.
- Situe la frontière presque apprenable de `idea_0107` dans la partie haute de la zone intéressante.
- Donne une forme particulière à la relation sujet-forme de `idea_0084`.
- Est dynamisée par les deux dérives de `idea_0122`.
